...
class Api {
    ...
    init(configuration: ApiConfiguration) {
        ...
        self.provider = ApiRequestProvider(configuration: configuration)
    }
    ...
    func vehicleStatus(_ vehicleId: UUID) async throws -> VehicleStatusResponse {
        let response: VehicleStatusResponse = try await provider.request(endpoint: .vehicleStatus(vehicleId)).response()
        return response
    }
    
    func vehicleCachedStatus(_ vehicleId: UUID) async throws -> VehicleStatusResponse {
        try await provider.request(endpoint: authorization?.isCcuCCS2Supported == true ? .vehicleCachedCCS2Status(vehicleId) : .vehicleCachedStatus(vehicleId)).response()
    }
...
private extension Api {
    func loginPage() async throws -> (userId: UUID, body: String) {
        ...
        let queryItems: [URLQueryItem] = [
            ...
            .init(name: "redirect_uri", value: try ApiRequest.url(for: .loginRedirect, configuration: configuration).absoluteString),
    ...
    func loginRedirect(userId: UUID, userIntegration: UserIntegrationResponse, sessionState: UUID) async throws {
        let queryItems: [URLQueryItem] = [
    ...
    @discardableResult
    func signIn(userId: UUID) async throws -> String {
        let headers: ApiRequest.Headers = [
            "Sec-Fetch-Site": "same-origin",
            "Sec-Fetch-Mode": "cors",
            "Sec-Fetch-Dest": "empty",
